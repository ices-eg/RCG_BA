...
```{r}
#the most basic function possible for dummy values
ratioIncProbFun <- function(estObj) {
  warning("Using dummy inclusion probability function that returns 1 for all fishIds")
  uniqFishIds <- unique(estObj$fishId)
  return(setNames(rep(1, length(uniqFishIds)), uniqFishIds))
}
source("R/createOverviewTbl.R")
```
...
```{r}
h1 <- spfs[[paste0("H",1)]]
h1e <- createOverviewTbl(h1, ratioIncProbFun)
h1e
```

```{r}
h2 <- spfs[[paste0("H",2)]]
h2e <- createOverviewTbl(h2, ratioIncProbFun)
h2e
```

```{r}
h3 <- spfs[[paste0("H",3)]]
h3e <- createOverviewTbl(h3, ratioIncProbFun)
h3e
```
...
```{r}
h8e <- createOverviewTbl(h8, ratioIncProbFun)
h8e
```
...
```{r}
mergers <- c(area = "CLarea")

agg_res <- aggregate_two_measures(
  count_formula = weight ~ area ,
   count_data    = estTbl[!is.na(estTbl$weight), ],
   count_fun     = length,
   sum_formula   = CLsciWeight ~ CLarea,
   sum_data      = cl$CL,
   sum_fun       = sum,
   merge_by      = mergers
)
...
agg_res <- aggregate_two_measures(
  count_formula = weight ~ area +  species,
   count_data    = estTbl[!is.na(estTbl$weight), ],
   count_fun     = length,
   sum_formula   = CLsciWeight ~ CLarea+ CLspecCode,
   sum_data      = cl$CL,
   sum_fun       = sum,
   merge_by      = mergers
)
...
mergers <- c(area = "CLarea", 
             country = "CLvesFlagCou",
             species = "CLspecCode")

agg_res <- aggregate_two_measures(
  count_formula = weight ~ area +  species + country,
   count_data    = estTbl[!is.na(estTbl$weight) , ],
   count_fun     = length,
   sum_formula   = CLsciWeight ~ CLarea+ CLspecCode + CLvesFlagCou,
   sum_data      = cl$CL,
   sum_fun       = sum,
   merge_by      = mergers
)
...
sum_label = "CL Scientific Weight (t)",
count_label = "Number of Weight Measurements",
facet = "CLvesFlagCou ~ speciesName "
)
...
plt + theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  coord_cartesian(ylim = c(0, 1000)) + labs(subtitle = "Zoomed in to max 1000 samples")
