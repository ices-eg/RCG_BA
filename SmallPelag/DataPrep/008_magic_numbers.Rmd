...
kable(summarise(group_by(filter(ca_spf, !(is.na(Age))), Year, FlagCountry), no_trips = length(unique(CS_TripId))))

kable(summarise(group_by(filter(ca_spf, !(is.na(Age)))), mean_no_trips = (length(unique(CS_TripId)))/2))

kable(summarise(group_by(filter(ca_spf, !(is.na(Age))), FlagCountry), mean_no_trips = mean(length(unique(CS_TripId)))/2))
