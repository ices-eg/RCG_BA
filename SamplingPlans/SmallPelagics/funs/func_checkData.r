...
# v3 09-05-2019 extended adaptations to project "rcg_ba_small_pelagics": new vslFlgCtry, 6 new rectangles, 3 new sppCodes and 2 sppNames, 2 metiers
# v3 09-05-2019 added argument ignore_stop: allows function to run and check all errors without stopping
# v3 09-05-2019 added argument "ignore_stop": allows function to run and check all errors without stopping
# v3 09-05-2019 added check on landCat
# v3 07-06-2019 fixed cat of depTab and landTab
# v3 08-06-2019 restricted check daysAtSea>20 to active gears (within frame)
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
...
dates <-as.character(seq(as.Date("2016-12-01"), as.Date("2019-01-31"), "days"))

active_gears <- c("OTB","PTM","PTB","SDN","OTM")

landCats <- c("HUC","IND",NA)
...
if(any(daysAtSea >20 & substring(b$foCatEu6,1,3) %in% active_gears, na.rm=T) )
{
longTrips <-unique(b$fishTripId[which(daysAtSea>20 & substring(b$foCatEu6,1,3) %in% active_gears)])
...
# landCat 
if(all(b$landCat %in% landCats)){cat("OK landCat in code list \n")}
if(!(all(b$landCat %in% landCats)))
{
cat("ERROR landCat not in the code list\n")
mislandCats <-table(b$landCat[which(!(b$landCat %in% landCats))], useNA="al")
print(mislandCats) 
}
